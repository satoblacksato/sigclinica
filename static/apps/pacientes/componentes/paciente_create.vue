<template>
  <div class="modal fade" id="paciente_form" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel"><i class="fa fa-user"></i>  Nuevo Paciente</h4>
      </div>
      ...
      <div class="col-md-12">
        ...
        <p :class="{ 'control': true }">   
          <select  v-validate="'required'" name="obra_social" v-model="paciente.obra_social" class="form-control">
    ...
    methods: {
      guardarPaciente(){
        this.$validator.validateAll().then((result) => {
          if (result) {
            axios.post(this.api.pacientes, this.paciente)
            .then((response) => 
            {
                this.mensaje('', 'El Paciente se guardo de forma correcta');
                this.paciente.nombre = '';
                this.paciente.apellido = '';
                this.paciente.documento = '';
                this.paciente.foto = '';
                this.paciente.tipo_documento = '';
                this.paciente.telefono_celular = '';
                this.paciente.telefono_fijo = '';
                this.paciente.observacion = '';
                this.$emit('cargar');
                $('#paciente_form').modal('hide');
            })
          }
        })
      },
